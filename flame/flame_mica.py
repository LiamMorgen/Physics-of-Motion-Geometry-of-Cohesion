...
class FLAME_mica(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.faces = flame_model.f # 和 flame 原版代码统一，方便可视化
    ...
    def get_facial_region_mask(self, vertices, region='mouth'):
        """
        获取面部特定区域的顶点掩码
        
        参数:
        vertices: 形状为 (batch_size, num_vertices, 3) 的张量,包含所有顶点的坐标
        region: 字符串,指定要获取的面部区域 ('mouth', 'eyes', 或 'both')
        
        返回:
        region_mask: 形状为 (batch_size, num_vertices) 的布尔张量,表示每个顶点是否属于指定的面部区域
        """
        # 获取所有面部特征点的坐标
        lmk_faces_idx = self.lmk_faces_idx.unsqueeze(0).expand(vertices.shape[0], -1)
        lmk_bary_coords = self.lmk_bary_coords.unsqueeze(0).expand(vertices.shape[0], -1, -1)
        landmarks = self._vertices2landmarks(vertices, self.faces, lmk_faces_idx, lmk_bary_coords)
        
        if region == 'mouth' or region == 'both':
            mouth_mask = self._get_mouth_mask(vertices, landmarks)
        
        if region == 'eyes' or region == 'both':
            eye_mask = self._get_eye_mask(vertices, landmarks)
        
        if region == 'both':
            return mouth_mask | eye_mask
        elif region == 'mouth':
            return mouth_mask
        elif region == 'eyes':
            return eye_mask
        else:
            raise ValueError("Invalid region specified. Choose 'mouth', 'eyes', or 'both'.")

    def _get_mouth_mask(self, vertices, landmarks):
        # 嘴部特征点索引
        mouth_landmark_indices = list(range(3, 10))
        
        # 选择嘴部的特征点
        mouth_landmarks = landmarks[:, mouth_landmark_indices, :]
        
        # 计算嘴部区域的边界框
        min_coords, _ = torch.min(mouth_landmarks, dim=1)
        max_coords, _ = torch.max(mouth_landmarks, dim=1)

        # 计算平移量（向下平移）
        shift_amount = 0.12 #0.12 
        shift_vector = torch.tensor([0, -shift_amount, 0], device=landmarks.device)
        min_coords = min_coords + shift_vector
        max_coords = max_coords + shift_vector

        # 调整下边界线向上移动
        bottom_adjustment = 0.07 # 0.08
        min_coords[:, 1] += bottom_adjustment

        # 调整右边界线向内收缩
        right_adjustment = 0.01 # 0.02
        max_coords[:, 0] -= right_adjustment
        
        # 创建嘴部区域的掩码
        mouth_mask = ((vertices >= min_coords[:, None, :]) & (vertices <= max_coords[:, None, :])).all(dim=-1)
        
        return mouth_mask

    def _get_eye_mask(self, vertices, landmarks):
        # 眼睛特征点索引 (需要根据FLAME模型的具体定义来调整)
        left_eye_indices = list(range(40, 44))  # 假设左眼特征点索引
        right_eye_indices = list(range(44, 48))  # 假设右眼特征点索引
        ...
        # 计算左眼和右眼的边界框
        left_min, _ = torch.min(left_eye_landmarks, dim=1)
        left_max, _ = torch.max(left_eye_landmarks, dim=1)
        right_min, _ = torch.min(right_eye_landmarks, dim=1)
        right_max, _ = torch.max(right_eye_landmarks, dim=1)
        ...
        left_min += shift_vector
        left_max += shift_vector
        right_min += shift_vector
        right_max += shift_vector
        
        # 创建眼睛区域的掩码
        left_eye_mask = ((vertices >= left_min[:, None, :]) & (vertices <= left_max[:, None, :])).all(dim=-1)
        right_eye_mask = ((vertices >= right_min[:, None, :]) & (vertices <= right_max[:, None, :])).all(dim=-1)

...
class Pytorch3dRasterizer(nn.Module):
    ...
    def forward(self, vertices, faces, attributes=None):
        ...
        # rasterize_meshes : https://pytorch3d.org/docs/renderer_getting_started

        # import pdb; pdb.set_trace()
        # 通过面片相对于像素的可见性，来选择需要用到的顶点位置
        ...
        pix_to_face[mask] = 0
        idx = pix_to_face.view(N * H * W * K, 1, 1).expand(N * H * W * K, 3, D) # torch.Size([16384, 3, 3])
        pixel_face_vals = attributes.gather(0, idx).view(N, H, W, K, 3, D) # torch.Size([1, 128, 128, 1, 3, 3])
        pixel_vals = (bary_coords[..., None] * pixel_face_vals).sum(dim=-2) # torch.Size([1, 128, 128, 1, 3]) # 像素值对应的重心坐标位置bary_coords
        pixel_vals[mask] = 0  # Replace masked values in output.
        pixel_vals = pixel_vals[:, :, :, 0].permute(0, 3, 1, 2) # torch.Size([1, 3, 128, 128]) # 像素值对应的gaussian点初始化位置
        pixel_vals = torch.cat([pixel_vals, vismask[:, :, :, 0][:, None, :, :]], dim=1) # torch.Size([1, 4, 128, 128])  # 像素值对应的gaussian点初始化位置 concat 可见性
...
class Embedder(nn.Module):
    ...
    def forward(self, inputs, alpha = 10.):
        ...
        if inputs.dim() == 3:
            ...
            inputs = inputs.permute(0, 2, 3, 1).reshape(batch_size, H*W, 3)
            
            # Apply embedding functions
            output = torch.cat([fn(inputs) for fn in self.embed_fns], -1)
            
            # Reshape output back to (batch_size, dim_embedded, H, W)
            output = output.reshape(batch_size, H, W, self.dim_embeded).permute(0, 3, 1, 2)
...
class ModifiedEmbedder(nn.Module):
    def __init__(self, N_freqs, input_dims=3, include_input=True):
        super().__init__()
        self.log_sampling = True
        self.periodic_fns = [torch.sin, torch.cos]
        self.max_freq = N_freqs - 1
        ...
        self.embed_fns = embed_fns
        self.dim_embedded = self.input_dims * len(self.embed_fns)

    def forward(self, inputs):
        # Reshape inputs to (batch_size, H*W, 3)
        batch_size, _, H, W = inputs.shape
        inputs = inputs.permute(0, 2, 3, 1).reshape(batch_size, H*W, 3)
        ...
        # Reshape output back to (batch_size, dim_embedded, H, W)
        output = output.reshape(batch_size, H, W, self.dim_embedded).permute(0, 3, 1, 2)

...
# from ..JunLi_2D import FLAMEToFeature

# ## 调试 Unet.py 文件的时候用这个
sys.path.append('..')
# from buildingblocks import DoubleConv, create_decoders, create_encoders
# from JunLi_2D import FLAMEToFeature, FLAMEToFeature3D
...
class AbstractUNet(nn.Module):
    ...
    def forward(self, x, flame):   # 多加一个 FLAME 参数
        # encoder part
        encoders_features = []
        ...
        # decoder part
        decoders_features = []
        for decoder, encoder_features, flame_feature in zip(self.decoders, encoders_features, flame):  # 多加一个 FLAME 参数
...
if __name__ == '__main__':
    ...
    flame = torch.randn(1, 120).to(device)
    flame_to_feature = FLAMEToFeature().to(device)
    flame_feature = flame_to_feature(flame)

    for i, tensor in enumerate(flame_feature):
        print(f"Shape of tensor {i}: {tensor.shape}")
    
    data_input = torch.randn(2,1,64,64,64).to(device)
    output = model(data_input)
    '''
        
    model = UNet2D(in_channels=3, out_channels=10, final_sigmoid=True)
    model.to(device)
    ...
    data_input = torch.randn(1,3,128,128).to(device)
    output = model(data_input, flame_feature)
    ...
    '''
    若要改为 3d
    输入从(1,3,128,128)  变为 (1, 3 ,3,128,128)

    flame 从 (1, 120) 变为 (1,3,120)

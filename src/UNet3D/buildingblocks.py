...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding, is3d):
    ...
    # print(f"Creating conv layer with order: {order}")
...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_features, x, flame_feature):
        # import pdb; pdb.set_trace()
        x, flame_feature = self.upsampling(encoder_features=encoder_features, x=x, flame_feature=flame_feature)
        x = self.joining(encoder_features, x, flame_feature)
        x = self.basic_module(x)
        return x

    @staticmethod
    def _joining(encoder_features, x, flame_feature, concat):
        if concat:
            return torch.cat((encoder_features, x, flame_feature), dim=1)
        else:
...
def create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups, is3d):
    ...
    for i in range(len(reversed_f_maps) - 1):
        if basic_module == DoubleConv:
            in_feature_num = reversed_f_maps[i] + reversed_f_maps[i + 1] + (32 // (2 ** i)) ##  添加 flame 参数
        else:
...
class AbstractUpsampling(nn.Module):
    ...
    def forward(self, encoder_features, x, flame_feature):
        ...
        return self.upsample(x, output_size), self.upsample(flame_feature, output_size)
